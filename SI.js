...
function pridobiPopulacijo() {
    ...
    okuzbe = parsedData[drzava]; 
    populacija = podatkiDrzave[drzava].populacija;
...
//---------------------------------------------------------------- GRAF POPVPREČJA --------------------------------------------------------------
class GrafPovprecja {
    constructor(idPlatna, maxGrafX, maxGrafY, dolžinaOkna) {
        this.can1 = document.getElementById(idPlatna); // v html delu poiščemo platno1; spremenljivka can1 sedaj predstavlja platno1
        this.plat1 = this.can1.getContext("2d"); // od tu dalje delamo s spremenljivo plat1 (za izris na platnu iz id "platno1")
    ...
    izriši(yVrednost) {
        ...
        this.plat1.font = "15px Arial";
        this.plat1.strokeStyle = "#000000";
        this.plat1.strokeText("Graf povprečja", 10, 20);

        this.plat1.beginPath(); // začetek izrisa
        this.plat1.strokeStyle = "#ff0000";

        for (var i = 0; i < this.y.length; i++) {
            // Normalize y values to ensure the graph starts at max y and reaches the top of the canvas
            const normalizedY = (this.y[i] / this.maxGrafY);
            const x = (this.x[i] / this.maxGrafX) * this.širinaPlatna;
            const yTop = this.višinaPlatna - (normalizedY * this.višinaPlatna);
...
function pridobiRealnePodatke() {
    if (cas > index * (Math.floor(stopCas/okuzbe.length))){
        if (index <= okuzbe.length - 1) {
            ...
            okuzeniR = okuzbe[index];
            dovzetniR = (populacija - okuzbe[index]);
            document.getElementById("dovzetniR").innerHTML = populacija - okuzbe[index];
            document.getElementById("okuzeniR").innerHTML = okuzbe[index];
            index++;
        }
    }
}
...
//---------------------------------------------------------------- SI MODEL ----------------------------------------------------------------------
var okuzeniSI = 0;
var dovzetniSI, dovzetniSICap;
var ustvarjeniSI = false;
function preracunajSI(DeltaTime = 0.001) {
    // Izračun števila dovzetnih
    if (okuzbe[index] !== 0 && ustvarjeniSI===false){
        okuzeniSI = okuzbe[index];
        ustvarjeniSI = true;
    }
    dovzetniSI = populacija - okuzeniSI;
    dovzetniSICap = okuzbe[okuzbe.length-1] - okuzeniSI;
    // Izračun novih okužb z upoštevanim časovnim korakom
    const noveOkužbeSI = infektivnost * okuzeniSI* intenzivnostKontaktov * dovzetniSICap / (dovzetniSICap+okuzeniSI)*DeltaTime;
    ...
    document.getElementById('okuzeniSI').innerHTML = Math.floor(okuzeniSI);
    document.getElementById('dovzetniSI').innerHTML = Math.floor(dovzetniSI);

    // Posodobimo grafe
    return Math.floor(okuzeniSI), Math.floor(dovzetniSI);
}
//-----------------------------------------------------------------------------------------------------------------------------------------------
...
var SSESI = 0;
function izracunajSSESI(realni_podatki, podatki_modela) {
...
async function OP() {
    ...
    const realDataSteps = 142; // Number of real data steps
    const realData = okuzbe;
    ...
    const fixedIntenzivnost = 20; // Fixed contact intensity
    const deltaTime = 0.1; // Fixed deltaTime
    ...
    // Log results
    console.log("Infektivnost:", infektivnost);
    console.log("Intenzivnost Kontaktov (fixed):", optimalIntenzivnost);
    console.log("DeltaTime (fixed):", optimalDeltaTime);

    // Apply optimal parameters
    applyOptimalParams(infektivnost, optimalIntenzivnost, optimalDeltaTime);
}
...
function optimizeSIModelParams(infektivnost, realData, stopČas) {
    ...
    for (let intenzivnost = 1; intenzivnost <= 20; intenzivnost += 1) {
        for (let deltaTime = 0.001; deltaTime <= 0.5; deltaTime += 0.001) {
            ...
            console.log("simulated Data at end", simulatedData[simulatedData.length-1]);
            console.log("real Data at end", okuzbe[okuzbe.length-1]);
            if (SSE < bestScore && SSE !== NaN && simulatedData[simulatedData.length-1] >= okuzbe[okuzbe.length-1]) {
                console.log("SSE:", SSE);
                console.log(simulatedData[simulatedData.length-1]);
                bestScore = SSE;
    ...
    console.log(`Najdeni optimalni parametri: Intenzivnost=${bestIntenzivnost}, DeltaTime=${bestDeltaTime},SSE=${bestScore}`);
...
function applyOptimalParams(infektivnost, intenzivnost, deltaTime) {
    ...
    preracunajSI.deltaTime = deltaTime; // Apply deltaTime globally
}
...
function inicializacija() {
    ...
    grafPovprecja = new GrafPovprecja("povprecje", 5000, 0.05, steviloAgentov);
...
// --------------------------------------------------------------- ZAGONSKE FUNKCIJE ---------------------------------------------------------------
var aktiven, stevecRdecih, stevecZelenih;
...
function zanka() {
    ...
    if( drzava === 'Australija') {okuzeniAgenti = stevecRdecih*3; dovzetniAgenti = stevecZelenih*3;}
    izracunajSSESI(okuzeniR, okuzeniSI);
    ...
    grafOkuzeni.dodajVrednost(okuzeniAgenti, okuzeniR, okuzeniSI); // dodamo vrednost v graf
    grafDovzetni.dodajVrednost(dovzetniAgenti, okuzbe[okuzbe.length-1]-okuzeniR, dovzetniSICap);
    grafDovzetni.izrisi();
    grafOkuzeni.izrisi(); // izris grafa
    grafPovprecja.izriši(številoTrkovRdečegaZzelenimVkoraku);
